...
fn main() -> io::Result<()> {
    ...
    while x < 1000 {
      let r = bfs(&graph, &mut flip_flops, &mut cons);
      total_low += r.0;
      total_high += r.1;
      x+= 1;
    }
...
fn bfs(graph: &HashMap<String, Vec<String>>, ff: &mut HashMap<String, bool>, cons: &mut HashMap<String, HashMap<String, bool>>) -> (usize, usize) {
    ...
    while !q.is_empty() {
        ...
        if graph.contains_key(&signal.0) {
          for x in  graph.get(&signal.0).unwrap().iter() {
              ...
              if send {high_count += 1;} else {low_count += 1;}
